...
unsigned long mt6897_power_state_dump_data[POWER_STATE_DUMP_DATA_SIZE];
...
static void consys_print_irq_status(void)
{
	...
	if (consys_check_conninfra_on_domain_status_mt6897() != 0)
		return;
	/*
...
#define CONN_TICK_TO_SEC(TICK) (TICK / CONN_32K_TICKS_PER_SEC)
	...
	if (print_log) {
		mt6897_power_state_dump_data[0] = round;
		mt6897_power_state_dump_data[1] = CONN_TICK_TO_SEC(conninfra_sleep_time);
		mt6897_power_state_dump_data[2] = CONN_TICK_TO_SEC((conninfra_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[3] = conninfra_sleep_cnt;
		mt6897_power_state_dump_data[4] = CONN_TICK_TO_SEC(wf_sleep_time);
		mt6897_power_state_dump_data[5] = CONN_TICK_TO_SEC((wf_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[6] = wf_sleep_cnt;
		mt6897_power_state_dump_data[7] = CONN_TICK_TO_SEC(bt_sleep_time);
		mt6897_power_state_dump_data[8] = CONN_TICK_TO_SEC((bt_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[9] = bt_sleep_cnt;
		mt6897_power_state_dump_data[10] = CONN_TICK_TO_SEC(gps_sleep_time);
		mt6897_power_state_dump_data[11] = CONN_TICK_TO_SEC((gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[12] = gps_sleep_cnt;
		mt6897_power_state_dump_data[13] = CONN_TICK_TO_SEC(t_conninfra_sleep_time);
		mt6897_power_state_dump_data[14] = CONN_TICK_TO_SEC((t_conninfra_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[15] = t_conninfra_sleep_cnt;
		mt6897_power_state_dump_data[16] = CONN_TICK_TO_SEC(t_wf_sleep_time);
		mt6897_power_state_dump_data[17] = CONN_TICK_TO_SEC((t_wf_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[18] = t_wf_sleep_cnt;
		mt6897_power_state_dump_data[19] = CONN_TICK_TO_SEC(t_bt_sleep_time);
		mt6897_power_state_dump_data[20] = CONN_TICK_TO_SEC((t_bt_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[21] = t_bt_sleep_cnt;
		mt6897_power_state_dump_data[22] = CONN_TICK_TO_SEC(t_gps_sleep_time);
		mt6897_power_state_dump_data[23] = CONN_TICK_TO_SEC((t_gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6897_power_state_dump_data[24] = t_gps_sleep_cnt;
...
#define CONN_GPT2_CTRL_AP_EN	0x38
	 ...
	 */
	if (consys_sema_acquire_timeout_mt6897(CONN_SEMA_THERMAL_INDEX, CONN_SEMA_TIMEOUT)
		== CONN_SEMA_GET_FAIL) {
