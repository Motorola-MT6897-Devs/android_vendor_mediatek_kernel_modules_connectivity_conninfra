...
unsigned long mt6989_power_state_dump_data[POWER_STATE_DUMP_DATA_SIZE];
...
#define CONN_TICK_TO_SEC(TICK) (TICK / CONN_32K_TICKS_PER_SEC)
	...
	if (print_log) {
		mt6989_power_state_dump_data[0] = round;

		mt6989_power_state_dump_data[1] = CONN_TICK_TO_SEC(conninfra_sleep_time);
		mt6989_power_state_dump_data[2] = CONN_TICK_TO_SEC((conninfra_sleep_time
							    % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6989_power_state_dump_data[3] = conninfra_sleep_cnt;

		mt6989_power_state_dump_data[10] = CONN_TICK_TO_SEC(gps_sleep_time);
		mt6989_power_state_dump_data[11] = CONN_TICK_TO_SEC((gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6989_power_state_dump_data[12] = gps_sleep_cnt;

		mt6989_power_state_dump_data[13] = CONN_TICK_TO_SEC(t_conninfra_sleep_time);
		mt6989_power_state_dump_data[14] = CONN_TICK_TO_SEC((t_conninfra_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6989_power_state_dump_data[15] = t_conninfra_sleep_cnt;

		mt6989_power_state_dump_data[22] = CONN_TICK_TO_SEC(t_gps_sleep_time);
		mt6989_power_state_dump_data[23] = CONN_TICK_TO_SEC((t_gps_sleep_time
							     % CONN_32K_TICKS_PER_SEC) * 1000);
		mt6989_power_state_dump_data[24] = t_gps_sleep_cnt;
...
int consys_power_state_dump_mt6989(char *buf, unsigned int size)
{
...
void consys_set_mcu_control_mt6989(int type, bool onoff)
{
