...
int consys_init_atf_data_mt6989_atf(void)
{
	...
	arm_smccc_smc(MTK_SIP_KERNEL_CONNSYS_CONTROL, SMC_CONNSYS_INIT_ATF_DATA_OPID,
		platform_config, consys_co_clock_type_mt6989(), 0, 0, 0, 0, &res);
	ret = res.a0;
	initialized = 1;
	return ret;
}
...
int consys_spi_read_mt6989_atf(enum sys_spi_subsystem subsystem, unsigned int addr,
	unsigned int *data)
{
	struct arm_smccc_res res;
	int ret;
...
int consys_spi_write_mt6989_atf(enum sys_spi_subsystem subsystem, unsigned int addr,
				unsigned int data)
{
	int ret = 0;
...
int consys_spi_update_bits_mt6989_atf(enum sys_spi_subsystem subsystem, unsigned int addr,
...
int consys_power_state_dump_mt6989_atf(char *buf, unsigned int size)
{
	...
	if(snprintf(buf, size,"[consys_power_state][round:%lu]"
		"conninfra:%lu.%03lu,%lu;gps:%lu.%03lu,%lu;"
		"[total]conninfra:%lu.%03lu,%lu;gps:%lu.%03lu,%lu;",
